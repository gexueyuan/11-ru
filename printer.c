...
void initialUART1(void)
{
   ...
   // #endif
     ...
     TRISFbits.TRISF5   = 0 ;;//u1tx output
     Nop();
	 TRISEbits.TRISE8=1;//u1rx input
#else	 
...
unsigned char MHType[] = {0x20, 0x20, 0x20,0x20, 0x20,
	                      0x80, 0x8B, 0x8A, 0x8E, 0x92, 0x85,0x8A, 0x92, 0x8E, 0x90, 0x0d};
	                            


    CopyXCHARtoCHAR(&PrintDataBuff[Datanum],MHType,16);//30-57
 ...
 {  0x20, 0x20, 
	...
	'P','R','O','-','1','0','0',' ','t','o','u','c','h','-','M',0x0d
 };


    CopyXCHARtoCHAR(&PrintDataBuff[Datanum],Table_Text_Alcovisor_Mercury,18);//30-57
...
void Timeto_Calibration()
{
  ...
  // datetemp=EEPROMReadByte(inspectYearAddress);
   TempCalibrationBuff[2]=Printbuff[253];//datetemp;//((datetemp/10)&0x0f)<<4+(datetemp%10);
   //datetemp=EEPROMReadByte(inspectMonthAddress);
   TempCalibrationBuff[1]=Printbuff[254];//datetemp;//((datetemp/10)&0x0f)<<4+(datetemp%10);
   //datetemp=EEPROMReadByte(inspectDayAddress);
   TempCalibrationBuff[0]=Printbuff[255];//datetemp;//((datetemp/10)&0x0f)<<4+(datetemp%10);
...
void Print_Table_Text_NONGDU()
{
    ...
    unsigned char Table_Text_mg100[] = //"g/L"
    {0x67, 0x2F, 0x4c, 0x0d};
    unsigned char Beyond300mgl[] = {'>','3','.','0','0',' ',0};
...
const XCHAR NoAlocholstr[] = {0x00AD, 0x00A5, 0x0020, 0x00AE, 0x00A1, 0x00AD, 0x00A0, 0x00B0, 0x00B3, 0x00A6, 0x00A5, 0x00AD, 0x0000};
	...
	if(Flashbuff[16] &ST_FastTest)/*快测*/
		{
		if(Printbuff[16]&ST_alocholTRUE){/*有酒精*/
			CopyXCHARtoCHAR(&PrintDataBuff[Datanum],alcoholdetect,sizeof(alcoholdetect));
			Datanum+=sizeof(alcoholdetect);
			}
		else{
			CopyXCHARtoCHAR(&PrintDataBuff[Datanum],NoAlocholstr,sizeof(NoAlocholstr));
			Datanum+=sizeof(NoAlocholstr);	
			}	
		}
    else if(Flashbuff[15] == 0xaa)//>3.000mg/l
		{
		CopyXCHARtoCHAR(&PrintDataBuff[Datanum],Beyond300mgl,6);
		Datanum+=6;
		CopyXCHARtoCHAR(&PrintDataBuff[Datanum],Table_Text_mg_l,5);
		Datanum+=5;
		}
 ...
 #ifdef TOW_DECIMAL Byte_to_PRN_TextBuffer(' ',4);
#else		 Byte_to_PRN_TextBuffer(temp_nongduL+'0',4);
#endif
		 Byte_to_PRN_TextBuffer(' ',5); 
...
void Print_Table_Text_GPS()
{
    ...
    CopyXCHARtoCHAR(&PrintDataBuff[Datanum],Table_Text_GPS,4);
...
unsigned char iii = 1;
   ...
   PrintDataBuff[Datanum++] = EEPROMReadByte(DebugADDRESS);
   PrintDataBuff[Datanum++] = EEPROMReadByte(DebugADDRESS+1);
   PrintDataBuff[Datanum++] = EEPROMReadByte(DebugADDRESS+2);

   for(iii = 1;iii <= 16;iii++)
       PrintDataBuff[Datanum++] = Printbuff[iii];

    PrintDataBuff[Datanum++] = EEPROMReadByte(MarkYrDataAddress);//last cali

    PrintDataBuff[Datanum++] = EEPROMReadByte(MarkMthDataAddress);

    PrintDataBuff[Datanum++] = EEPROMReadByte(MarkDayDataAddress);



	PrintDataBuff[Datanum++] = EEPROMReadByte(inspectYearAddress);//last check

    PrintDataBuff[Datanum++] = EEPROMReadByte(inspectMonthAddress);

    PrintDataBuff[Datanum++] = EEPROMReadByte(inspectDayAddress);
    ...
    CopyXCHARtoCHAR(&tx1_buff[2],&PrintDataBuff,Datanum);

	
	
    tx1_ptrend   += Datanum+2;
