...
#define ID_BUTTONYE_P   143
#define ID_BUTTONYE_M   145
#define ID_BUTTONMO_P   146
#define ID_BUTTONMO_M   147
#define ID_BUTTONDA_P   148
#define ID_BUTTONDA_M   149
...
void CreateAllSetting();
WORD MsgAllSetting(WORD objMsg, OBJ_HEADER* pObj);
...
volatile unsigned int offcount=0;
...
BYTE TestMode=0xaa;  //测试类型aa=1,bb=2,cc=3
...
volatile unsigned int PrintCount=0;
unsigned char tt;//use for USB
#define OutPio4 _TRISE9 = 0//配对管脚输入输出设置
#define InPio4 _TRISE9 = 1
#define HPio4  PORTEbits.RE9=1
#define LPio4  PORTEbits.RE9=0
#endif
...
{
      EEPROMWriteByte(0x11, MarkYrDataAddress);//实际校准
...
EEPROMWriteByte(0x31, PrintSelectAddress);//under 0
...
/*
    EEPROMWriteWord(15998,EESERIAL_NUMBER);
    EEPROMWriteWord(0,Record_over_16000);
    Record_Over_Flag=0;	*/
}
...
if(PeakScreen==0xaa)
   ...
   Flashbuff[15] = 0;
    ...
    Nop();           
    Nop();
    if(serialnumber > Code_MAX){ serialnumber = 1;		
          EEPROMWriteWord(Code_OVER,Record_over_16000);	
          Record_Over_Flag = EEPROMReadWord(Record_over_16000);
          serialnumberRecord = Code_MAX;
        //  serialnumber++;
    }
...
GOL_MSG msg;                    // GOL message structure to interact with GOL
    ...
    BEEP_TIME = 200;//EEPROMReadWord(Beepsld1Address);
    TIMER_BASE = 400;//EEPROMReadWord(Beepsld2Address);
...
//	if((serialnumber == 1)&&(Record_Over_Flag == 0xaaaa))//
//	serialnumberRecord=1;//
    ...
    if(Tem_Detect<=0x0088)
    {
      ...
      OutTextXY(30,100,TemperatureExtremelow);
      //while(!OutText(TemperatureExtremelow));
      DelayMs(2000);
      POWER_CONTROL = 0;//开关机
    	}
    ...
    else if(Tem_Detect>=0x0139)
    	{
         ...
         // MoveTo(30,100);
          SetColor(BRIGHTRED);
          SetFont((void*)&FONTMEDIUM);// FONTMEDIUM
         // while(!OutText(TemperatureExtremehigh));
		  OutTextXY(30,100,TemperatureExtremehigh);
          DelayMs(2000);
          POWER_CONTROL = 0;//开关机
    }
   ...
   // alt5Scheme->CommonBkColor = WHITE;
    
    navScheme->pFont = (void*)ptrLargeAsianFont;

    
    //mainScheme->CommonBkColor = WHITE;
...
EEPROMWriteByte(0X86,DebugADDRESS);
EEPROMWriteByte(0X00,DebugADDRESS+1);
EEPROMWriteByte(0X05,DebugADDRESS+2);
...
//-----写入仪器号---------
/*
EEPROMWriteByte(0X12,ID_JIQIADDRESS);
EEPROMWriteByte(0X03,ID_JIQIADDRESS+1);
EEPROMWriteByte(0X70,ID_JIQIADDRESS+2);
EEPROMWriteByte(0X00,ID_JIQIADDRESS+3);
*///-------
    while(1){
...
//          return MsgSetDate(objMsg, pObj, pMsg); 
    ...
    case DISPLAY_MASTERPASSWORD:
        ...
        case DISPLAY_AUTOOFF:
            return 1;
        case DISPLAY_AllSet:
             return MsgAllSetting(objMsg, pObj);
...
static  BYTE offnum=0;
    ...
    if ((screenState & 0x0000F300) != 0x0000F300) {     // process only when NOT setting time and date
        ...
        if(sLatitude[11]!=0&&sLongitude[12]!=0)//&&exdisplay!=0xee)
             {                     
              PutImage(20,5,(void*)&gpslogo,IMAGE_NORMAL);
...
#ifdef Bluetooth
        if((PORTCbits.RC1)&&(!Bluetoothflag))
    		{
    		
                PutImage(20,3,(void*)&bluetooth,IMAGE_NORMAL);
                Bluetoothflag = 0xaa;
                //U1BRG=103;
            }
        else if((!PORTCbits.RC1)&&(Bluetoothflag))
        	  {
...
goto_test:      if((!directionZYQ))
    ...
    case CREATE_RECORD:
        ...
        case DISPLAY_RECORD:
#ifdef Bluetooth
            if((!PORTCbits.RC1)&&(WndGetText(pWin)==RecordStr))
        		{
                  //SetColor(YELLOW);
            ...
            else if((PORTCbits.RC1)&&(WndGetText(pWin)==RecordBKStr))
        		{
            ...
            if((ChoicePrint)&&(GetState(pbtn,BTN_DRAW)))
        		{
            ...
            if(S4_count >10)	//
              ...
              if(Record_Over_Flag != Code_OVER)
              {
                ...
                else if(serialnumberRecord==serialnumber-1)	
        	    {
        	  	...
        	  	{
                    if(serialnumberRecord<=Code_MAX-10)
        				{
                         serialnumberRecord +=10;//;= serialnumber
                         screenState = CREATE_RECORD;//  
        			}
                    else if((serialnumberRecord>Code_MAX-10)&&(serialnumberRecord<=Code_MAX)){
                         serialnumberRecord -=Code_MAX-10;//;= serialnumber
                         screenState = CREATE_RECORD;//
        			}
            ...
            if(S4_count >10)	//
              {
                ...
                if(Record_Over_Flag != Code_OVER)
                    ...
                    serialnumberRecord += (Code_MAX-10);
                    screenState = CREATE_RECORD;
        		}
                }
        else if(serialnumberRecord==1)	
...
//-------    	
while(S4_count <30){    // 等待放开
            BTN_S3 = 1;DelayMs(10);         
            if(BTN_S3 == 0){S4_count = 0;}            
            else{S4_count ++;}
            } 		       
//-------
//      while(!BTN_S3); 
...
//-------   
S4_count = 0;   
while(S4_count <10){    // 等待按键
            BTN_S3 = 1;DelayMs(10);         
            if(BTN_S3 == 0){S4_count ++;}             
            else{S4_count = 0;}	
            }S4_count = 0;	 	
while(S4_count <30){    // 等待放开
            BTN_S3 = 1;DelayMs(10);         
...
case DISPLAY_AllSet:
    ...
    case DISPLAY_BlueTooth:
            if(GetState(pbutton, BTN_DISABLED))
             ...
             if(StGetText(PST1)==PrintMstr)
            //if(strcmp(StGetText(PST1),PrintMstr)==0)
                if(PORTCbits.RC1)
        			{
                        StSetText(PST1, BluetoothMstr);
                        SetState(PST1, ST_DRAW);
            	    }
    			
            if(StGetText(PST1)==BluetoothMstr)
            //if(strcmp(StGetText(PST1),BluetoothMstr)==0)
                if(!PORTCbits.RC1)
        			{
                      StSetText(PST1, PrintMstr);
                      SetState(PST1, ST_DRAW);	
        		}
    			
            if(StGetText(PST)==Nonestr)
            //if(strcmp(StGetText(PST),Nonestr)==0)
                if(PORTCbits.RC1)
        			{
            ...
            //if(strcmp(StGetText(PST),Workstr)==0)
                if(!PORTCbits.RC1)
        			{
...
if(screenState==CREATE_RECORD)
            BtnCreate(ID_BUTTON_HOME,
              63,GetMaxY()-41,                      // left, top corner	
                   103,GetMaxY()-2,0,           // right, bottom corner (with radius = 0)
                   BTN_DRAW,                // will be dislayed after creation
                   (void*)&HOME,                       	// no bitmap	
                  NULL,//, HomeStr          // LEFT arrow as text
                  blackScheme); 
...
#define EB1STARTX       7//2	//月
#define EB2STARTX       82//75	//日
#define EB3STARTX       157//130	//年
#define EBSTARTY    	65
#define EBSTARTY1       110
#define EB4STARTX       15	//时
#define EB5STARTX       90	//分
#define EB6STARTX       165	//秒
#define EBSTARTY2       165
...
void CreateDateTime() {
    ...
    BtnCreate(ID_BUTTON_YR,             // button ID 
		...
		BtnCreate(ID_BUTTONDA_P,			// button ID 
				  EB1STARTX+textWidth+1,
				  EBSTARTY,
				  EB1STARTX+textWidth+SELECTBTNWIDTH+1,
		...
		BtnCreate(ID_BUTTONDA_M,			// button ID 
				  EB1STARTX+textWidth+1,
				  EBSTARTY+(textHeight>>1)+1,
				  EB1STARTX+textWidth+SELECTBTNWIDTH+1,
				  EBSTARTY+textHeight, 			
				  0,						// draw rectangular button	
				  BTN_DRAW, 				// will be dislayed after creation
		...
		EbCreate(ID_EB_MONTH,				// ID
				  ...
				  2,						// max characters is 3
				  altScheme);				// use alternate scheme
		BtnCreate(ID_BUTTONMO_P,			// button ID 
		...
		BtnCreate(ID_BUTTONMO_M,			// button ID 
				  EB2STARTX+textWidth,
				  EBSTARTY+(textHeight>>1)+1,
				  EB2STARTX+textWidth+SELECTBTNWIDTH+1,
				  EBSTARTY+textHeight, 			
...
textWidth = (tempStrWidth*3) +(GOL_EMBOSS_SIZE<<1); 
		EbCreate(ID_EB_YEAR,				// ID
				  ...
				  4,						// max characters is 4
				  altScheme);				// use alternate scheme
		BtnCreate(ID_BUTTONYE_P,			// button ID 
				  EB3STARTX+textWidth+1,
				  EBSTARTY,
				  EB3STARTX+textWidth+SELECTBTNWIDTH+1,
				  EBSTARTY+(textHeight>>1),			
		...
		BtnCreate(ID_BUTTONYE_M,			// button ID 
				  EB3STARTX+textWidth+1,
				  EBSTARTY+(textHeight>>1)+1,
				  EB3STARTX+textWidth+SELECTBTNWIDTH+1,
				  EBSTARTY+textHeight, 			
  ...
  /*  
    BtnCreate(    ID_BUTTON4,           // button ID 
                  EB6STARTX-45,
                  EBSTARTY2+2*textHeight+5,
                  EB6STARTX+textWidth+SELECTBTNWIDTH,
                  EBSTARTY2+3*textHeight+5,          	
...
WORD MsgDateTime(WORD objMsg, OBJ_HEADER* pObj) {
	...
	char   year = 0, month  = 0, day    = 0;
    WORD   btnID; 
    WORD   ebID = 0, i;
    //static XCHAR  tempString[3];
	//-------------luokuizhang---
	static WORD memory_objMsg=OBJ_MSG_INVALID;
	if(memory_objMsg==objMsg)return 1;
	else memory_objMsg=objMsg;
	//------------------
    ...
    switch(btnID = GetObjID(pObj)) {
        ...
        case ID_BUTTON4:                            // exit setting of date and time
            if(objMsg == BTN_MSG_RELEASED){
                screenState = CREATE_AllSet;            // goto last state screen
                return 1;
            }
            break;
 ...
 //------------
		...
		case ID_BUTTONYE_P:  						// increment hour value
		    if(objMsg == BTN_MSG_PRESSED){
				year = +1;
				ebID = ID_EB_HOUR;
				i = 4;								
			...
			break;
        case ID_BUTTONYE_M:							// decrement hour value
			if(objMsg == BTN_MSG_PRESSED){
				year = -1;
				ebID = ID_EB_HOUR;
				i = 4;
			...
			break;
		case ID_BUTTONMO_P:  						// increment hour value
		    if(objMsg == BTN_MSG_PRESSED){
				month = +1;
				ebID = ID_EB_HOUR;
				i = 4;								
			...
			break;
        case ID_BUTTONMO_M:							// decrement hour value
			if(objMsg == BTN_MSG_PRESSED){
				month = -1;
				ebID = ID_EB_HOUR;
				i = 4;
			...
			break;
		case ID_BUTTONDA_P:  						// increment hour value
		    if(objMsg == BTN_MSG_PRESSED){
				day = +1;
				ebID = ID_EB_HOUR;
				i = 4;								
			...
			break;
        case ID_BUTTONDA_M:							// decrement hour value
			if(objMsg == BTN_MSG_PRESSED){
				day = -1;
				ebID = ID_EB_HOUR;
				i = 4;
...
//--------------   		
    ...
    if (ebID) {   									// check if time was modified
		mRTCCOff();
    	RTCCSetBinYear(mRTCCGetBinYear() + year);	// update RTCC registers
    	RTCCSetBinMonth(mRTCCGetBinMonth() + month);
    	RTCCSetBinDay(mRTCCGetBinDay() + day);
...
unsigned char ik;
    switch(GetObjID(pObj)){
        ...
        case ID_BUTTON2:
            ...
            else   screenState = CREATE_Alcoholdetect;
   ...
   */         
        ...
        case ID_BUTTON3:
            ...
            if(objMsg == BTN_MSG_RELEASED)//BTN_MSG_PRESSED)
    		{
                    ...
                    Bluetoothflag = 0;
            	#endif
               if((serialnumberRecord>0)||(Record_Over_Flag ==Code_OVER));              //保证不写记录时候不显示
                   {
                    if(serialnumberRecord==0)
                        if(Record_Over_Flag ==Code_OVER)serialnumberRecord=Code_MAX;
                    	else return 1;
...
mainScheme->pFont = (void*)&GOLFontDefault;
    SetColor(COMMONBACKGROUNDCOLORDEFAULT);//(WHITE); //WHITE
    ClearDevice();  
...
void Aircheck(){
    ...
    SetColor(COMMONBACKGROUNDCOLORDEFAULT); //WHITE
    ClearDevice(); 
    SetColor(COMMONBACKGROUNDCOLORDEFAULT); //WHITE
...
/*
                ...
                */
        		// 
                MarkTemp = 0;
                if((consbuffStr[4]>0x30)&&(consbuffStr[4]<0x3a))
                ...
                for(i=1,j=1;i<=4;i++)
                    {
                        if(consbuffStr[4-i]!=0x2e)
                        	{
                              j*=10;
                                if((consbuffStr[4-i]>0x30)&&(consbuffStr[4-i]<0x3a))
...
//                  EEPROMWriteByte(TestMiMaStr[0] , PrintTimesAddress);
    ...
    switch(id){
        ...
        case ID_BUTTON_BACK:
            if(objMsg == BTN_MSG_RELEASED){
                screenState = CREATE_AllSet;    // goto list box screen
            }
            return 1; // process by default
...
void CreateRecord()
{
    char temp,temp1;
    unsigned int i;
const XCHAR alcoholdetect[] = {0x00AE, 0x00A1, 0x00AD, 0x00A0, 0x00B0, 0x00B3, 0x00A6, 0x00A5, 0x00AD, 0x0000};    //
const XCHAR NoAlocholstr[] = {0x00AD, 0x00A5, 0x0020, 0x00AE, 0x00A1, 0x00AD, 0x00A0, 0x00B0, 0x00B3, 0x00A6, 0x00A5, 0x00AD, 0x0000};
    for(i=0;i<256;i++)
    Flashbuff[i]=0;
    readrecord();
    SaveReadItem(FALSE);
    ...
    StCreate(ID_STATICTEXT3,                // ID 
              ...
              mainScheme);                   // use alternate scheme    
if(ST_IRstatus&(ST_REFUSE))/*拒绝测试(超时)*/
...
StCreate(ID_STATICTEXT3,                // ID 
              ...
              mainScheme);    //   alt2Scheme            // use alternate scheme
  else if(ST_IRstatus&ST_Passive)/*手动*/
    StCreate(ID_STATICTEXT3,                // ID 
              positionax+3*positionmaxx,positionay+3*positionmaxy,
              7*(positionax+positionmaxx),positionay+4*positionmaxy,                // dimension
              ST_DRAW,//|ST_RIGHT_ALIGN,//ST_CENTER_ALIGN,      // display text
  ...
  else if(ST_IRstatus&ST_Discontinued)/*吹起中断*/
    StCreate(ID_STATICTEXT3,                // ID 
              positionax+3*positionmaxx,positionay+3*positionmaxy,
              7*(positionax+positionmaxx),positionay+4*positionmaxy,                // dimension
              ST_DRAW,//|ST_RIGHT_ALIGN,//ST_CENTER_ALIGN,      // display text
    ...
    else if(ST_IRstatus &ST_FastTest)/*快测*/
    {
        if(ST_IRstatus &ST_alocholTRUE)/*有酒精*/
    	{
        StCreate(ID_STATICTEXT3,                // ID 
              positionax+3*positionmaxx,positionay+4*positionmaxy,
              7*(positionax+positionmaxx),positionay+5*positionmaxy,                // dimension
              ST_DRAW,//|ST_RIGHT_ALIGN,//ST_CENTER_ALIGN,      // display text
              alcoholdetect,//GetdataStr,// SubjectNameStr,                 // "TEST", 	// text
              mainScheme);  
    	}
        else/*无酒精*/
    	{
        StCreate(ID_STATICTEXT3,                // ID 
              ...
              7*(positionax+positionmaxx),positionay+5*positionmaxy,                // dimension
              ST_DRAW,//|ST_RIGHT_ALIGN,//ST_CENTER_ALIGN,      // display text
              NoAlocholstr,//GetdataStr,// SubjectNameStr,              // "TEST", 	// text
              mainScheme);  
    	}
    }	
    else
...
if(Flashbuff[15] == 0xaa)
        ...
        StCreate(ID_STATICTEXT3,                // ID 
              ...
              Beyond300mgl,//GetdataStr,// SubjectNameStr,              // "TEST", 	// text
              mainScheme);
...
StCreate(ID_STATICTEXT3,                // ID 
  ...
  if(!(ST_IRstatus &ST_FastTest))/*快测*/           
    pbtn = (OBJ_HEADER*)BtnCreate(ID_BUTTON8,               // button ID 
              126, KEYSTARTY+5*KEYSIZEY+40,
              166,KEYSTARTY+6*KEYSIZEY+40,          // dimension
              0,                       	// set radius 
       ...
       case ID_BUTTON_HOME:
        ...
        case ID_BUTTONChina_P:
            ...
            if(Record_Over_Flag != Code_OVER)
                ...
                if(serialnumberRecord<Code_MAX)
                {
                    serialnumberRecord ++;//;= serialnumber
                    screenState = CREATE_RECORD;//
                }
                else if(serialnumberRecord == Code_MAX)
                {
                    serialnumberRecord = 1;
                    screenState = CREATE_RECORD;    // goto list box screen
                }	
...
WORD MsgSetting(WORD objMsg, OBJ_HEADER* pObj){
    ...
    switch(GetObjID(pObj)){
        ...
        case ID_BUTTON_NEXT:
            if(objMsg == BTN_MSG_RELEASED){
               screenState = CREATE_AllSet;// goto check box demo screen
            }
            return 1;                           // process by default

        case ID_BUTTON_BACK:
            if(objMsg == BTN_MSG_RELEASED){
                screenState = CREATE_AllSet;        // goto ECG demo screen
            }
            return 1;                           // process by default
    ...
    //---------
        ...
        //  Marktemporary=MarkAdc;
           // EEPROMWriteWord(MarkAdc,0x6022);
            ...
            EEPROMWriteByte((unsigned char)consbuffStr[0], PreMarkcon_1_ADDRESS);//consbuffStr
            EEPROMWriteByte((unsigned char)consbuffStr[2], PreMarkcon_2_ADDRESS);
            EEPROMWriteByte((unsigned char)consbuffStr[3], PreMarkcon_3_ADDRESS);
            EEPROMWriteByte((unsigned char)consbuffStr[4], PreMarkcon_4_ADDRESS);
       ...
       case ID_BUTTON_HOME:
        ...
        case  ID_BUTTON1:
         ...
         if(objMsg == BTN_MSG_RELEASED){
              ...
              if(TestMode==0xcc)ST_IRstatus|=ST_FastTest;
...
//  if(TestMode!=0xcc)
    ...
    if(PeakScreen ==0xaa)
            StCreate(ID_STATICTEXT3,           		// ID 
                      ...
                      5*(positionax+positionmaxx),positionay+9*positionmaxy,                // dimension
                      ST_DRAW,//|ST_RIGHT_ALIGN,//ST_CENTER_ALIGN,      // display text
                      Beyond300mgl,//GetdataStr,                // "TEST", 	// text
                    ...
                    {   	
                       if(Demarcate&&EEPROMReadByte(ID_CHECKBOXADDRESS+ID_CHECKBOX8))
                             screenState = CREATE_CALLOCK;
                               ...
                               TestMode=0xcc;ST_IRstatus|=ST_FastTest;
                                screenState = CREATE_Waittest;      // goto ECG demo screen
                               status_Press=FALSE;		
                               for(ik=0;ik<200;ik++) Flashbuff[49+ik] = 0;//clear  buffer --add by xiao 090811 
                               g_backlight_time = 3*60;// 3分钟不吹气，自动跳出来
...
void CreateThickness(){
    ...
    RbCreate(ID_RADIOBUTTONStar + 26,           // ID 
              positionax+0*positionmaxx,(GetMaxY() - 140),positionax+3*positionmaxx,(GetMaxY() - 110),           // dimension
              RB_DRAW|RB_GROUP,//|  \
              //RB_CHECKED,                 // will be dislayed and checked after creation
                                        // first button in the group
    ...
    RbCreate(ID_RADIOBUTTONStar + 28,          // ID 
              ...
              positionax+6*positionmaxx,(GetMaxY() - 110),           // dimension
              RB_DRAW,                 // will be dislayed after creation
              K2100StrA,               // "Center"
              alt5Scheme);              // use alternate scheme
    ...
    RbCreate(ID_RADIOBUTTONStar + 29,          // ID 
              ...
              positionax+6*positionmaxx,(GetMaxY() - 80),           // dimension
              RB_DRAW,                 // will be dislayed after creation
              K2150StrA,               // "Center"
              alt5Scheme);              // use alternate scheme
    ...
    RbCreate(ID_RADIOBUTTONStar + 32,          // ID 
             positionax+0*positionmaxx,(GetMaxY() - 80),positionax+3*positionmaxx,(GetMaxY() - 50),           // dimension
              RB_DRAW,                 // will be dislayed after creation
              K2300StrA,               // "Right"
              alt5Scheme);
...
DisplayPrecon[0] = (XCHAR)EEPROMReadByte(PreMarkcon_1_ADDRESS);
DisplayPrecon[1] = '.';
DisplayPrecon[2] = (XCHAR)EEPROMReadByte(PreMarkcon_2_ADDRESS);
DisplayPrecon[3] = (XCHAR)EEPROMReadByte(PreMarkcon_3_ADDRESS);
DisplayPrecon[4] = (XCHAR)EEPROMReadByte(PreMarkcon_4_ADDRESS);
DisplayPrecon[5] = 0;
    ...
    StCreate(ID_STATICTEXT3,                // ID 
              ...
              ST_DRAW|ST_CENTER_ALIGN,      // display text
              DisplayPrecon,//consbuffStr,              // "TEST", 	// text
              mainScheme);
    StCreate(ID_STATICTEXT3,                // ID 
              5*positionmaxx,60,
...
/*
        case ID_BUTTON7:
            if(objMsg == BTN_MSG_RELEASED){
               ...
               screenState =CREATE_Choosegas; //CREATE_PASSWORD;            // goto Demarcate demo screen
            }
            return 1;                           // process by default
...
void  CreateBlueTooth()
{
    ...
    if(PORTCbits.RC1)
    	{
            PST1 = StCreate(ID_STATICTEXT3,             	// ID 
                     4,75,
                     236,113,               // dimension
                     ST_DRAW|ST_CENTER_ALIGN,       // display text
                     PrintMstr,                 // "TEST", 	// text
